...
set -ex
apt-get update
apt-get $APT_INSTALL_ARGS libtool make cmake libseccomp-dev gcc python3 python3-venv
EOS
...
EOF
apt-get update
apt-get $APT_INSTALL_ARGS $needed
...
useradd -u 901 -r -s /sbin/nologin compiler
useradd -u 902 -r -s /sbin/nologin code
useradd -u 903 -G code -r -s /sbin/nologin spj
...
HEALTHCHECK --interval=5s --retries=3 CMD .venv/bin/python3 /app/service.py
EXPOSE 8080
